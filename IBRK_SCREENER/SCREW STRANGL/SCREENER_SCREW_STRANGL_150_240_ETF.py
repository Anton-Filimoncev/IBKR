...
for tick in ticker_list:
    ...
    # ДАТЫ ЭКСПИРАЦИИ
    limit_date_min = datetime.datetime.now() + relativedelta(days=+150)
    ...
    # atm_strike = nearest_equal(put_df['strike'].tolist(), current_price)
    # atm_put_volatility = put_df[put_df['strike'] == atm_strike]['iv'].reset_index(drop=True).iloc[0]
